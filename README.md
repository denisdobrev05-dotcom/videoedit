...
## 2. Уеб интерфейс (най-лесно) 🌐

Ако не ти се пишат команди, има локален сайт. Пускаш го веднъж и работиш през
браузъра — качваш видео, избираш FPS, сваляш резултата. Обработката тече
**локално на твоя GPU**, нищо не се качва в интернет.
...
После отвори в браузъра: **<http://localhost:5000>**

В интерфейса можеш да:

- издърпаш (drag & drop) или избереш видео файл;
- избереш режим **Целеви FPS** или **Множител**;
- избереш модел, качество (CRF) и UHD режим;
- следиш прогрес бар на живо (Извличане → Интерполация → Кодиране);
- свалиш готовия `.mp4` (запазва се и в папка `outputs/`).

> При първо пускане, ако `rife-ncnn-vulkan` липсва, сайтът се опитва да го свали
> автоматично в `./bin/`. ffmpeg трябва да е инсталиран предварително (т. 1.2).
> Ако нещо липсва, в горната част на страницата ще видиш ясно съобщение.

За скриптовия (CLI) вариант — продължи надолу.

## 2б. Употреба от командния ред (CLI)
